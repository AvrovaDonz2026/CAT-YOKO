...
_CE_CHUNK_CACHE: dict[tuple, int] = {}
...
def _ce_chunk_tokens(
    ...
    if device.type != "cuda" or not torch.cuda.is_available():
        return 512
    budget = _ce_logit_budget_bytes(device, dtype)
    key = (str(device), int(vocab), int(n_tok), int(budget))
    ...
    try:
        ...
        return 512
    max_chunk = int(free // (budget * max(int(vocab), 1)))
...
def linear_cross_entropy(
    ...
    *,
    ...
    step = _ce_chunk_tokens(h.size(0), vocab, chunk_tokens, h.device, h.dtype)
