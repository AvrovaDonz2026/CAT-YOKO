...
class WrapPolicyTests(unittest.TestCase):
    def test_b0_wraps_frozen_skips_student_and_router(self) -> None:
        ...
        self.assertNotIsInstance(model.lm_head, Nvfp4Linear)
        self.assertNotIsInstance(model.decoder[0].self_attn.q_proj, Nvfp4Linear)
        self.assertNotIsInstance(model.decoder[0].mlp.experts[0].gate_proj, Nvfp4Linear)
        ...
        self.assertTrue(all(n.startswith("encoder.") for n in names))
