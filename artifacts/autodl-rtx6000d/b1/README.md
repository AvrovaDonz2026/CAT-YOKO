...
| Item | Value |
| --- | --- |
| Envelope (published) | 27e9 tokens; this script is a GPU smoke test, not a completed 27B run |
...
| resume | `/root/autodl-tmp/runs/b0-full` (if overlay present) else `/root/autodl-tmp/runs/b0` |
...
DummyStream; do not load 50B corpus. Do not pull GitHub. Do not write passwords / deploy keys into this directory. Weights go to HuggingFace only, not GitHub.

Do not grab the GPU while 6000D is running published B0; run this script after B0 finishes.
