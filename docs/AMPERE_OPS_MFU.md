...
4. **torch inductor 32 workers** 抢 CPU。`TORCH_COMPILE_DISABLE=1`。`CUDA_DEVICE_MAX_CONNECTIONS=32`。MoE top-k 顺序每步都变，ZeRO 按上次 trace 预取会 miss、SM 空约 1s。`leaf_module` 把 `MoE` / `EncoderBlock` / `DecoderBlock` 当整块 prefetch。DeepSpeed inflight H2D cap 提到 8 后空窗更密（1.38/min），保持默认 2。`LOG_EVERY=40`。不 persist 专家。
