...
CUTLASS 把 SM100A `stg.256` 编进 kernel 的条件是 **nvcc ≥ 12.9**。CUDA 12.8 编出来的 core：16×128 FPROP 能过，B0 尺寸（4096×2048）的 NVFP4 quantize 会打 `CUTE_ARCH_STORE256_SM100A_ENABLED` 然后把 GPU 打挂。Vast 上 `apt-get install cuda-nvcc-12-9 libcublas-dev-12-9 cuda-nvtx-12-9`（12.9 nvcc 默认没有 cuBLAS / NVTX）。`MAX_JOBS` 默认 `nproc`（这台 192 vCPU），不要钉死 8。`te_linear_ready` 用 **4096×2048** 探（B0 seq），不再用 16×128 / 256×2048。启动脚本和源码编译脚本都把 CUDA 12.9 `lib64` 放进 `LD_LIBRARY_PATH`。
