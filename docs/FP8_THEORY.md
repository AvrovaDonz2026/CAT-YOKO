...
> 规格仍是中间档：16/24，≈12B / 2.3B-in / 4.5B-out。C1 冻结边界不变。**Phase B 墙钟按 C1+FP8 定稿。** 不引入新架构，不写训练骨架。
...
**Phase B 墙钟配方按 C1+FP8 定稿：761 H100-h，联合 bf16 的 56%。**

联合 bf16（1,354）只是 100% 对照，不是要跑的路径。C1 bf16（1,090）是操作数账，仍按 bf16 计 6NT。真正发布、真正要跑的是 **C1 的 token 切分 × 混合 FP8 策略**。全阶段套 1.5×（726）和峰值 2×（545）不定稿。

中间档训练的 **相当一部分** 是 MoE 专家 GEMM：占存储参数的 **84.6%**、占计入 6NT 的 **71.5%**。这块（外加冻结 Encoder 的推理 GEMM）走 FP8。FP8 **不改** Kaplan \(6N_{\mathrm{act}}T\)，只提高 H100 上完成这些 FLOPs 的有效吞吐。

| 做法 | 50B tok H100-h | vs 联合 bf16 | 角色 |
| --- | ---: | ---: | --- |
| 两栈一起训，bf16 | 1,354 | 100% | 对照 |
| C1 解冻课程，bf16 | 1,090 | 81% | 操作数账 |
| **C1+FP8**（B0 student bf16；B1/B2 MoE GEMM 与冻结 Encoder 前向 1.5×） | **761** | **56%** | **定稿** |
| C1 全阶段套 1.5×（B0 student 也 FP8） | 726 | 54% | 敏感性 |
| C1 × 峰值 2× | 545 | 40% | 上界，不发布 |
...
8. **C1 与 FP8 相乘，不是相加。** C1 先砍 19% FLOPs，再在剩下的墙上乘 1.5×（B0 例外）。相对联合 bf16：\(81\% \times\)（B1/B2 1.5×，B0 近 1×）≈ **56%**。这条积就是定稿墙钟 **C1+FP8 = 761**。
...
相对联合 bf16 1,354：**56%**。相对 C1 bf16 1,090：再砍约 **30%** 墙钟。**761 是 Phase B 发布墙钟。**
...
| 发布 C1+FP8 ≈761（≤60% 联合 bf16） | PASS（761 / 1,354 = 56%） |
...
1. **Phase B 墙钟按 C1+FP8 定稿：761 H100-h。** 联合 bf16 1,354 只作对照；C1 bf16 1,090 只作操作数账。
2. §6 / §8：精度改为 C1+FP8 模块策略。
3. §15.1：默认 Phase B 行是 C1+FP8 **761**；1,413 / 1,354 标为联合 bf16 对照。
4. §15.3：杠杆 #5+#6 的发布积是 761（56%）。
