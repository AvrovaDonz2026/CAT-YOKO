...
\(S=1\) 就是计划 §15.1 的 bf16 列；C1 的 \(F_{\mathrm{C}}\) 相对联合少 21%，所以 bf16 墙钟从 1,325 降到 1,046。FP8 只进入 \(S\)。
...
## 2. 「相当一部分」：MoE GEMM 占 6NT 的 81.9%

中间档一次完整前向 \(N_{\mathrm{fwd}}=6.36\mathrm{B}\)（emb 与 head 各计一次）：

| 块 | 激活参数 | 占 6NT |
| --- | ---: | ---: |
| MoE 专家（top-\(k\) SwiGLU） | 5.21B | **81.9%** |
| 自注意力 + cross-attn 投影 | 0.61B | 9.7% |
| untied embedding + lm_head | 0.53B | 8.4% |
| RMSNorm / router / softmax | （6NT 忽略） | — |

存储侧 MoE 仍是 **90.6%**（882 专家槽，含未入 top-\(k\) 的副本）。训练 FLOPs 走的是激活，所以「相当一部分」按 **81.9%** 计。GQA 把注意力从旧占位的 24% 压到 9.7%，MoE 份额相应上升——Amdahl 只更好，不更差。
...
1.69 高于发布的 1.5，差额留给 dispatch/combine 与 12B 小 kernel。若连 attn 投影也 FP8，\(f=95.8\%\)，\(S\approx 1.92\)——这是上界，**不写入 §15.1**。

冻结 Encoder 的前向（B0/B1）是同一批 GEMM 的推理形态：无权重梯度、无激活反传，是整张图里最安全的 FP8 切口。B0 里它约占该阶段 FLOPs 的一小部分。
...
| 模块 | 为什么不能 FP8 |
| --- | --- |
| 输入 \(E_{\mathrm{in}}\) | 定理 E：B0/B1 冻结 Encoder 时 \(X^0=s_{\mathrm{emb}}\,\mathrm{onehot}\,E_{\mathrm{in}}\) 是冻结栈的输入分布。FP8 表会给 16 层冻结 MiniCPM5 残差加量化噪声。 |
| lm_head | untied 输出头；B1 可训，但仍是分类面，不进 FP8 GEMM 白名单。 |
...
| 假设 | H100-h | vs 联合 |
| --- | ---: | --- |
| B0 student 也 1.5× | 697 | 53% |
| 全阶段 MoE-Amdahl 1.69× | 617 | 47% |
| 峰值 2× | 523 | 39% |

不把 2× 写进计划。12B 上若实测 \(S<1.3\)，回退 B1/B2 到 bf16，C1 的 1,046 仍在。

冻结 Encoder 权重 4.38B：bf16 8.76 GB → FP8 存储 4.38 GB。这是推理副本；master 仍可 bf16。省的是带宽，不是 6NT。
...
- **C1 显存杠杆仍在**：B1 Adam 62%、detach 激活 ~38%（留下 26/42）。FP8 再砍 GEMM 激活带宽；不替代冻结。
...
| 发布 2× | 抄 H100 峰值 | 12B Amdahl 1.69；锁 1.5× |
| B0 student FP8 弄脏定理 A | gate 邻域 + 新模块 | B0 student bf16；只 FP8 冻结 Encoder 前向 |
| FP8 输入表漂冻结 Encoder | 训/量化 \(E_{\mathrm{in}}\) | 白名单 + 定理 E |
...
| C1 bf16 小时不被 FP8 策略改写 | PASS（1,046） |
...
5. §13：L0 保持 bf16；FP8 从 B1 开。
6. 不写训练代码骨架（按用户要求，理论先闭环）。
