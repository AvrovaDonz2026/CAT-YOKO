...
- **CUDA 烟测**：`python3 -m cat_yoko.gpu_smoke` 跑 tiny（含 encoder offload / CPU Adam / B0→B1→B2 链 / KD / dummy upcycle / eval / 跨阶段 resume / 2-rank gloo DDP + **CUDA DDP** / **CUDA C1 DDP** / **1-rank FSDP**）。`--middle` 在 ≥28GiB GPU 上对 **MiniCPM5 GQA 12.25B 图**做 C1 步（默认 1；`--steps N` 生效；bf16 直接建在 CUDA 上，禁止 CPU fp32 再 cast）。`--middle --phase B1` 冻 Encoder 卸载到 CPU + CPU Adam。`--c1` 同一张 12B 图上 B0→B1→B2（默认各 1 步）。B2 逐层 offload，backward 完一层就 clip+Adam 掉梯度。无卡 skip。4M global batch / 全参 GPU Adam 仍要多卡或 ZeRO。FSDP wrap 传 `device_id` 与 `use_orig_params=True`。ckpt extra 含 `cfg`/`seed`。在 RTX 4080 SUPER 32GB + 62GiB cgroup 上，**MiniCPM5-2B 图**独立进程 B0 一步 peak **22.84GiB**（23386MiB，params=12,250,381,312）。旧 MiniCPM-2B 占位图的 21.65 / 26.3 / 13.38 / 22.73 GiB **作废**，不得再当现行峰值。B1 / B2 / `--c1` 链与 `--middle --steps 2` 在进程间 teardown 之后复核。12B unittest 每个 case 释放图，禁止同一进程叠两份 12.25B。tiny CUDA `GpuTinyTests` 13 ok（含 2-rank CUDA DDP 与 1-rank FSDP）。
