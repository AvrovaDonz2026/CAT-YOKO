...
- **CUDA 烟测**：`python3 -m cat_yoko.gpu_smoke` 跑 tiny（含 encoder offload / CPU Adam / B0→B1→B2 链 / KD / dummy upcycle / eval / 跨阶段 resume / 2-rank gloo DDP + CUDA DDP / 1-rank FSDP）。`--middle` 在 ≥28GiB GPU 上对 **12B 图**做 C1 步（默认 1；`--steps N` 生效；bf16 直接建在 CUDA 上，禁止 CPU fp32 再 cast）。`--middle --phase B1` 冻 Encoder 卸载到 CPU + CPU Adam。`--c1` 同一张 12B 图上 B0→B1→B2（默认各 1 步）。B2 逐层 offload，backward 完一层就 clip+Adam 掉梯度。无卡 skip。4M global batch / 全参 GPU Adam 仍要多卡或 ZeRO。FSDP wrap 传 `device_id` 与 `use_orig_params=True`。ckpt extra 含 `cfg`/`seed`。在 RTX 4080 SUPER 32GB + 62GiB cgroup 上实测：`--c1 --seq-len 64` B0 peak **21.65GiB**、B1 **26.3GiB**、B2 **13.38GiB**（B2 gn≈25.5）；`--middle --steps 2` B0 GPU Adam 两步 peak **22.73GiB**，nll 254.06→253.97。
