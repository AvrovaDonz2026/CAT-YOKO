...
- **C–G 入口**：`python3 -m cat_yoko.c|d|e|f|g`（`scripts/train_{c-g}.py` / `cat-yoko-c`）。复用同一套 trainer + overlay。`--try` 仍是 32 步、seq=64。**C** `--stage indexer|topk|hca|win`（默认 indexer）：冻主干、bf16 Lightning Indexer **层内 KL** 对齐稠密滑窗注意力（不是穿过 YOCO cache 的 CE）；随后 top-k / HCA 是 **PyTorch SDPA 掩码 / KV 均值池化**，不是 CSA CUDA kernel。**D** `--stage 8k|32k|128k`。**E** WSD decay 到峰值 ~1/100。**F** SFT，prompt 段 `labels=-100`。**G** `--algo grpo|dpo`（步数信封，默认 10000；`--try` 用 dummy RLVR）。C–G 默认不下载 Ultra-FineWeb / UltraChat。
...
- 自研 CSA CUDA kernel / V4 式 FP4 专家存储 / **把 50B 语料检进 git** / 评测套件。数据接口吃 prepare 产出的 mmap `.bin`，或 jsonl `tokens` / SFT `labels`。Phase C 用 `cat_yoko.indexer.LightningIndexer` + 层内 KL，以及 PyTorch SDPA top-k 掩码 / HCA 均值池化；**不是** CSA kernel。
