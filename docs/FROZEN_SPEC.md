...
- **OpenBMB 数据路径**：`cat_yoko.prepare` 把思考 mix 打成 seq_len 对齐的 int32 mmap `.bin`：Ultra-FineWeb en/zh + UltraData-Math + **5% StarCoder**（默认 0.55/0.30/0.10/0.05）。可选 `phase-b-code` 为 10% 代码。不在 CI / 本 VM 下载语料。DummyStream 用同一 `THINK_CODE_FRAC` 掺仓库内短 snippet（utf-8 哈希铺行），不是 50B。tokenizer 默认 `openbmb/MiniCPM5-2B`（\(V=130560\)）。Trainer 对 `.bin` 走 `PackedBinStream`，并从 `*.bin.meta.json` 读 `eos_id`。`--upcycle-hf` / `--teacher-hf` 拉 `openbmb/MiniCPM5-2B-Base`。
