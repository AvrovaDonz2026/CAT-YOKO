...
- **C–G 入口**：`python3 -m cat_yoko.c|d|e|f|g`（`scripts/train_{c-g}.py` / `cat-yoko-c`）。复用同一套 trainer + overlay。`--try` 仍是 32 步、seq=64。**C** `--stage indexer|topk|hca|win`（默认 indexer）或 `--chain`：冻主干、bf16 Lightning Indexer **层内 KL** 对齐稠密滑窗注意力（只挂 Encoder CSA，不复用到 decoder query）；随后 CSA 是 **滑窗 ∪ 压缩块 top-k**（自身块排除，定理 B），HCA 是 **滑窗 concat 均值池化槽**，都不是 CSA CUDA kernel。**KDA**（`cat_yoko.kda`）默认关；`use_kda=True` / `--use-kda` 时 encoder 非 bootstrap 层 3:1 KDA:(CSA/HCA)、decoder self 3:1 KDA:sliding，用固定 `[H,d,d]` 循环状态替代这些层的序列 KV，**不缩小 YOCO 全局 cache**，不是 DPLR CUDA kernel。3:1 图的 Phase C 链是 **`C-kda` → indexer → topk → hca → win**（多数线性路径先点亮，CSA 锚点后开，HCA 最后）；CSA/HCA **代码仍在**，只是点亮靠后。默认 `use_kda=False` 仍是 indexer→topk→hca→win，B0 overlay 132 张量不变。**D** `--stage 8k|32k|128k` 或 `--chain`。**E** WSD decay 到峰值 ~1/100。**F** SFT，prompt 段 `labels=-100`（jsonl 可给 `prompt_ids`/`response_ids`）。**G** `--algo grpo|dpo`（步数信封，默认 10000；`--try` 用 dummy RLVR）。C–G 默认不下载 Ultra-FineWeb / UltraChat。
