...
- **CUDA 烟测**：`python3 -m cat_yoko.gpu_smoke` 跑 tiny（含 encoder offload / CPU Adam / B0→B1→B2 链）。`--middle` 在 ≥28GiB GPU 上对 **12B 图**做一步 C1 B0（bf16 直接建在 CUDA 上，禁止 CPU fp32 再 cast）。`--middle --phase B1` 冻 Encoder 卸载到 CPU + CPU Adam。`--c1` 同一张 12B 图上 B0→B1→B2 各一步；B2 逐层 offload，backward 完一层就 Adam 掉梯度。无卡 skip。4M global batch / 全参 GPU Adam 仍要多卡或 ZeRO。在 RTX 4080 SUPER 32GB + 62GiB cgroup 上实测：B0 peak 21.65GiB，B1 26.3GiB，B2 13.38GiB。
