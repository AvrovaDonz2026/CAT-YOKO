...
- **C–G 入口**：`python3 -m cat_yoko.c|d|e|f|g`（`scripts/train_{c-g}.py` / `cat-yoko-c`）。复用同一套 trainer + overlay。`--try` 仍是 32 步、seq=64。**C** `--stage indexer|topk|hca|win`（默认 indexer）或 `--chain`：冻主干、bf16 Lightning Indexer **层内 KL** 对齐稠密滑窗注意力（只挂 Encoder CSA，不复用到 decoder query）；随后 CSA 是 **滑窗 ∪ 压缩块 top-k**（自身块排除，定理 B），HCA 是 **滑窗 concat 均值池化槽**，都不是 CSA CUDA kernel。**D** `--stage 8k|32k|128k` 或 `--chain`。**E** WSD decay 到峰值 ~1/100。**F** SFT，prompt 段 `labels=-100`（jsonl 可给 `prompt_ids`/`response_ids`）。**G** `--algo grpo|dpo`（步数信封，默认 10000；`--try` 用 dummy RLVR）。C–G 默认不下载 Ultra-FineWeb / UltraChat。
